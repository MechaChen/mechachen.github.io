---
sidebar_label: '[Implement] 實作 react-redux'
title: '[Implement] 實作 react-reudx'
tags:
  - Implement
  - react-redux
  - Observer pattern
...
主要是因為使用 `useContext(context)` 時，還是會取到整個 context object，當 context object 有值變動時，就會 re-render，進而 create 一個新的 context object，就算用了 `useMemo` 也一樣
...
subgraph no-Memo
	direction TB
	id31["Parent component re-render"] --> id41["component 產生<br/>一個新的 context object"]
	id41 --> id51["context 的 reference 變了"]
...
subgraph useMemo
	...
	id42 --> id52["發現有變動，useMemo 產生<br/>一個新的 context object"]
	id52 --> id62["useContext 的 component <br />偵測到新的 context object reference"]
	id62 --> id72["因為 context 的 reference 變了，<br />使用該 context 的 component re-render"]
end
```
...
1. **整個 context object 的參考會一直變動，造成所有使用 context 的 component 一直 re-render**
...
react-reudx 的重頭戲，實現 `useSelector` 過程，主要是利用
